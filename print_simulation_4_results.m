...
for mf = 1:length(MotifNameall)
    ...
    for k = 1:length(N)
        for l = 1:length(N)
            Na = N(k);
            Nb = N(l);
            cover = readtable(strcat("./results/cover_edge_",   MotifName,"_",GraphonName1,"_",GraphonName2,    "NA_",string(Na), "NB_",string(Nb), "_mn_",string(m),".csv"));
            cover = table2array(cover);
            coveragemean(k,l) = nanmean(cover);
            coveragestd(k,l) = nanstd(cover,0);
            cover = readtable(strcat("./results/cover_norm_",   MotifName,"_",GraphonName1,"_",GraphonName2,    "NA_",string(Na), "NB_",string(Nb), "_mn_",string(m),".csv"));
            cover = table2array(cover);
...
% Print results for unmatched nodes pooling
