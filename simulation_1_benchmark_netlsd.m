	...
	for n = nall
		...
		for MotifName = MotifNameall
			...
			for i = 1:iterc
				...
				commandStr = sprintf('python subroutines/method_benchmark_netlsd.py "%s" "%s"', filename1, filename2);
			...
			writematrix(coverage,strcat("./results/cover_netlsd_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				"100shift_",sprintf('%03d',100*shift_amount),...
				"m_", string(m), "n_", string(n),".csv"))

			writematrix(time_cost,strcat("./results/time_cost_netlsd_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				"100shift_",sprintf('%03d',100*shift_amount),...
				"m_", string(m), "n_", string(n),".csv"))
		end
