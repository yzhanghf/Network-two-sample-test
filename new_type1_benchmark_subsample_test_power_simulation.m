...
% 				sparsity_parameters_b ^(-s)*sparsity_parameters_b ^s *mu_n;
			...
			for i = 1:iterc
				...
				tic;
				[t_stat, p_value] = method_benchmark_subsample(A,B,MotifName,N_boot);
				time_cost(i,1) = toc;
			...
			writematrix(coverage,strcat("./new-result-test-power/cover_subsample_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				"100shift_",sprintf('%03d',100*shift_amount),...
				"m_", string(m), "n_", string(n),".csv"))

			writematrix(time_cost,strcat("./new-result-test-power/time_cost_subsample_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				"100shift_",sprintf('%03d',100*shift_amount),...
				"m_", string(m), "n_", string(n),".csv"))
		end
