...
% 				sparsity_parameters_b ^(-s)*sparsity_parameters_b ^s *mu_n;
			...
			for i = 1:iterc
				...
				for query_index = 1:N_networks
					...
					% D_mn = zzz(1)-zzz(2);
					% s_mn = sqrt(zzz(3));
					% t_mn = (D_mn-true_D)/s_mn;
					% t_vec(query_index,1) = t_mn;
					% smn_vec(query_index,1) = s_mn;
					% alpha1_vec(query_index,1) = sqrt(zzz(4));
					% p_value_hat_vec(query_index,1) = 2*min( [ normcdf(t_mn), 1-normcdf(t_mn) ] );
		...
		writematrix(FDR_mean_record,strcat("./new-result-FDR/query_FDR_mean_",...
		...
		writematrix(FDR_sd_record,strcat("./new-result-FDR/query_FDR_sd_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				'Ntests_',sprintf('%d',N_networks), ...
				'_CalAquant_', sprintf('%2d', 100*calA_set_quantile), ...
				'_shift_', sprintf('%2d', 100*graphon_shift), ...
		...
		writematrix(power_mean_record,strcat("./new-result-FDR/query_power_mean_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				'Ntests_',sprintf('%d',N_networks), ...
				'_CalAquant_', sprintf('%2d', 100*calA_set_quantile), ...
				'_shift_', sprintf('%2d', 100*graphon_shift), ...
		...
		writematrix(power_sd_record,strcat("./new-result-FDR/query_power_sd_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				'Ntests_',sprintf('%d',N_networks), ...
				'_CalAquant_', sprintf('%2d', 100*calA_set_quantile), ...
				'_shift_', sprintf('%2d', 100*graphon_shift), ...
