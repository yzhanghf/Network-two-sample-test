...
function [t_stat, p_value, test_stat_output_1, test_stat_output_2] = method_benchmark_subsample(A,B,MotifName,N_boot)
	...
	for(ii = 1:N_boot)
		...
		rho_A_list(ii) = sum(sum(A_temp))/(mm*(mm-1));
		u_m_list(ii) = Motif(A_temp,0,MotifName);

		B_idx = randi(n,nn,1);
		B_temp = B(B_idx,B_idx);
		rho_B_list(ii) = sum(sum(B_temp))/(nn*(nn-1));
