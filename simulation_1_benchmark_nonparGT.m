	...
	for n = nall
		...
		for MotifName = MotifNameall
			...
			for i = 1:iterc
				...
				command = sprintf('Rscript subroutines/method_benchmark_nonparGT.R "%s" "%s"', filename1, filename2);
			...
			writematrix(coverage,strcat("./results/cover_nonparGT_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				"100shift_",sprintf('%03d',100*shift_amount),...
				"m_", string(m), "n_", string(n),".csv"))

			writematrix(time_cost,strcat("./results/time_cost_nonparGT_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				"100shift_",sprintf('%03d',100*shift_amount),...
				"m_", string(m), "n_", string(n),".csv"))
		end
