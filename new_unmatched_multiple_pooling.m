...
% GraphonName1 =  'SmoothGraphon2';  GraphonName2 =  'SmoothGraphon4';  sparsity_parameters_a = 0.325;  sparsity_parameters_b = 1.0;
...
% GraphonName1 =  'NewSmoothGraphon2';  GraphonName2 =  'NewSmoothGraphon4';  sparsity_parameters_a = 1.0;  sparsity_parameters_b = 1.0;
...
% % temp debug
% iterc=1; mall=[1000]; nall=[1000];
% MotifNameall = ["Triangle"];
% % MotifNameall = ["Vshape"];
...
% 				sparsity_parameters_b ^(-s)*sparsity_parameters_b ^s *mu_n;
			...
			t_mc = zeros(iterc,1);
			for i = 1:iterc
				smooth = randn(1)*sqrt(cdelta*(log(m)*(Na*m)^(-1/2)+log(n)*(Nb*n)^(-1/2)));

				A_tensor = zeros(m,m,Na);
				for(na_idx = 1:Na)
					X = rand(m,1);
					W1 = graphon(X,X,sparsity_parameters_a,GraphonName1);  W1 = W1-diag(diag(W1));
					A_tensor(:,:,na_idx) = generate_A(W1);
				end
				...
				for(na_idx = 1:Na)
					...
					%%%%%% NEW %%%%%
					a1_record 		= [a1_record, a1];
					rho_ahat_record = [rho_ahat_record, rho_ahat];
					u_mhat_record 	= [u_mhat_record, u_mhat];
					e_a4a1_record	= [e_a4a1_record, e_a4a1];
					e_a1a3_record	= [e_a1a3_record, e_a1a3];
					e_a3_record 	= [e_a3_record, e_a3];
					e_a1a1a2_record = [e_a1a1a2_record, e_a1a1a2];
					ksei_al2_record = [ksei_al2_record, ksei_al2];
					
				end

				e_a4a1  = mean(e_a4a1_record);
				e_a1a3  = mean(e_a1a3_record);
				e_a3 	= mean(e_a3_record);
				e_a1a1a2= mean(e_a1a1a2_record);
				ksei_al2= mean(ksei_al2_record);

				%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

				B_tensor = zeros(n,n,Nb);
				for(nb_idx = 1:Nb)
					Y = rand(n,1);
					W2 = graphon(Y,Y,sparsity_parameters_b,GraphonName2);  W2 = W2 + shift_amount;  W2 = W2-diag(diag(W2));
					B_tensor(:,:,nb_idx) = generate_A(W2);
				end
				...
				for(nb_idx 		= 1:Nb)
					...
					%%%%%% NEW %%%%%
					b1_record 		= [b1_record, b1];
					rho_bhat_record = [rho_bhat_record, rho_bhat];
					v_nhat_record 	= [v_nhat_record, v_nhat];
					e_b1b3_record	= [e_b1b3_record, e_b1b3];
					e_b4b1_record	= [e_b4b1_record, e_b4b1];
					e_b3_record 	= [e_b3_record, e_b3];
					e_b1b1b2_record = [e_b1b1b2_record, e_b1b1b2];
					ksei_bl2_record = [ksei_bl2_record, ksei_bl2];

				end

				e_b1b3  = mean(e_b1b3_record);
				e_b4b1  = mean(e_b4b1_record);
				e_b3 	= mean(e_b3_record);
				e_b1b1b2= mean(e_b1b1b2_record);
				ksei_bl2= mean(ksei_bl2_record);


				% calculating t ststistics and t hat
				s_mn2 = Na^(-2)*m^(-2)*sum(a1_record.^2) + Nb^(-2)*n^(-2)*sum(b1_record.^2);
				s_mn = sqrt(s_mn2);

				D_mn = mean(rho_ahat_record.^(-s).*u_mhat_record) - mean(rho_bhat_record.^(-s).*v_nhat_record);
				t_mc(i,1) = (D_mn)/s_mn;

				% calculating I and qm,n
				I0 = s_mn^(-1)*(1/(m*Na)*a0 -1/(n*Nb)*b0);

				q1 = 1/2*s_mn^(-3)*(-Na^(-2)*m^(-2)*e_a4a1 ...
					- Na^(-2)*m^(-2)*e_a1a3 + Nb^(-2)*n^(-2)*e_b1b3 ...
					+ Nb^(-2)*n^(-2)*e_b4b1);

				q2 = s_mn^(-3)*(Na^(-2)*m^(-2)*(1/6*e_a3+e_a1a1a2) - Nb^(-2)*n^(-2)*(1/6*e_b3+e_b1b1b2))+...
					1/2*s_mn^(-5)*((-Na^(-3)*m^(-3)*ksei_al2 - Na^(-2)*m^(-2)*Nb^(-1)*n^(-1)*ksei_bl2)*(e_a1a3 + e_a4a1)+...
						(Na^(-1)*m^(-1)*Nb^(-2)*n^(-2)*ksei_al2 + Nb^(-3)*n^(-3)*ksei_bl2)*(e_b1b3+e_b4b1));
			...
			writematrix(coverage,	strcat("./new-result-2-unmatched-multiple-pooling/cover_edge_",		MotifName,"_",GraphonName1,"_",GraphonName2,    "shift_", string(100*shift_amount),    "NA_",string(Na), "NB_",string(Nb), "_mn_",string(m),".csv"))
			writematrix(coverage2,	strcat("./new-result-2-unmatched-multiple-pooling/cover_norm_",		MotifName,"_",GraphonName1,"_",GraphonName2,    "shift_", string(100*shift_amount),    "NA_",string(Na), "NB_",string(Nb), "_mn_",string(m),".csv"))
			writematrix(t_mc,		strcat("./new-result-2-unmatched-multiple-pooling/teststat_",		MotifName,"_",GraphonName1,"_",GraphonName2,    "shift_", string(100*shift_amount),    "NA_",string(Na), "NB_",string(Nb), "_mn_",string(m),".csv"))
		end
	end % end for n
	fprintf('\n\n');
end
