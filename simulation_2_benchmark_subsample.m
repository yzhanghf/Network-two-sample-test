...
rec(:,1) = minmaxp;
rec(:,2) = Truthind;
writematrix(rec,strcat("./results/minpv_truth_subsamp_",example,string(n),".csv"));
writematrix(elapsedTime,strcat("./results/querytime_pval_subsamp_",example,string(n),".csv"));
writematrix(auc_r,strcat("./results/auc_pval_subsamp_",example,string(n),".csv"));
