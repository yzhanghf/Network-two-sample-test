...
Files = dir(strcat("./datasimulation/data_simulation_",string(n)));
isdir = [Files.isdir];
indexf = isdir ==0;
Filesf = Files(indexf);
...
for rndind = 1:total_query
    filesquery = strcat("./datasimulation/example_",string(n),'/',example,'_',string(rndind));
    load(filesquery);
    queryad = A;
...
rec(:,1) = minmaxp;
rec(:,2) = Truthind;
writematrix(rec,strcat("./ROC_result/minpv_truth_subsamp_",example,string(n),".csv"));
writematrix(elapsedTime,strcat("./ROC_result/querytime_pval_subsamp_",example,string(n),".csv"));
writematrix(auc_r,strcat("./ROC_result/auc_pval_subsamp_",example,string(n),".csv"));
