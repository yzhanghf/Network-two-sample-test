...
% 				sparsity_parameters_b ^(-s)*sparsity_parameters_b ^s *mu_n;
			...
			for i = 1:iterc
				...
				tic;
				[zzz1,zzz2,zzz3,zzz4] = method_benchmark_resample(A,B,MotifName,N_boot);
			...
			writematrix(coverage,strcat("./new-result-degenerate-CI-coverage/cover_resample_",...
			...
			writematrix(t_mc,strcat("./new-result-degenerate-CI-coverage/resample_MC_t_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				'100UstatReduPow_',sprintf('%03d',100*alpha), ...
				'_100sparA_',sprintf('%03d',1000*abs(A_sparse_power)), ...
				'_100sparB_',sprintf('%03d',1000*abs(B_sparse_power)), ...
	...
	writematrix(time_cost_mean,strcat("./new-result-degenerate-CI-coverage/resample_time_cost_mean_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				'100UstatReduPow_',sprintf('%03d',100*alpha),...
				'_100sparA_',sprintf('%03d',1000*abs(A_sparse_power)), ...
				'_100sparB_',sprintf('%03d',1000*abs(B_sparse_power)), ...
				".csv"))
	writematrix(time_cost_sd,strcat("./new-result-degenerate-CI-coverage/resample_time_cost_sd_",...
				MotifName,"_",GraphonName1,"_",GraphonName2,"_",...
				'100UstatReduPow_',sprintf('%03d',100*alpha),...
				'_100sparA_',sprintf('%03d',1000*abs(A_sparse_power)), ...
				'_100sparB_',sprintf('%03d',1000*abs(B_sparse_power)), ...
